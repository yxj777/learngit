...
---
## EXT2文件系统（inode）
- 以inode为基础的文件系统，档案系统一开始就将inode与block规划好，除非格式化（或resize2fs指令）<br>
否则inode与block固定后不再变动
- Ext2档案系统在格式化的时候基本上是区分为多个区块组（block group）,每个区块组合都有独立的 <br>
...
- ### 目录树读取
  - 文件名是记录在目录的 block 当中，当要读取某个档案时，就务必会经过目录的 inode 与 block ，然后才能够找到那个待读取文件的 inode 号码，最终才会读到正确的档案的 block 内的数据
...
- ### 日记文件系统
  ...
  - 2.&nbsp;实际写入：开始写入档案的权限与资料; 开始更新 metadata 的数据;
...
## Linux 文件系统的运作
- ### 异步处理（asynchronously）
  - 当系统加载一个文件到内存后，如果该档案没有被更动过，则在内存区段的档案数据会被设定为干净（clean）的
  - 但如果内存中的文件数据被更改过了，此时该内存中的资料会被设定为脏的 (Dirty)。 此时所有的动作都还在内存中执行，并没有写入到磁盘中
  - 但如果内存中的文件数据被更改过了（例如你用 nano 去编辑过此档案），此时该内存中的资料会被设定为脏的 (Dirty)。 此时所有的动作都还在内存中执行，并没有写入到磁盘中
- ### 内存与Linux
  - 系统会将常用的档案数据放置到主内存的缓冲区，以加速档案系统的读/写;
  - 因此 Linux 的实体内存最后都会被用光！ 这是正常的情况！ 可加速系统效能;
  - 你以手动使用 sync 来强迫内存中设定为 Dirty 的文件回写到磁盘中;
  - 若正常关机时，关机指令会主动呼叫 sync 来将内存的数据回写入磁盘内;
  - 但若不正常关机（如跳电、当机或其他不明原因），由于数据尚未回写到磁盘内， 因此重新启动后可能会花很多时间在进行磁盘检验，甚至可能导致档案系统的损毁（非磁盘损毁）
---
## 挂载点的意义 （mount point）
...
- ### XFS文件系统配置
  ...
  - #### 实时操作区（realtime section）
    - 当有档案要被建立时，xfs 会在这个区段里面找一个到数个的 extent 区块，将档案放置在这个区块内
